...
3. Design minimal data model: Organizations, Applications, Servers, Integrations, BusinessCapabilities, Relations with ArchiMate element/type metadata.
...
- Relations: generic typed edges (e.g., serves, depends_on, runs_on, implements, realizes); store ArchiMate element/type metadata for rendering.
...
- Resources (CRUD): /organizations, /applications, /servers, /integrations, /business-capabilities, /relations.
...
- Components/Schemas (representative): Organization, Application, Server, Integration, BusinessCapability, Relation, View, Graph (nodes, edges, styling), ImportJob, ExportJob, Webhook, Error.
- Paths (examples):
...
- Graph model: nodes (id, type, archimate_element, label, attrs, data refs), edges (id, source, target, archimate_relationship, relation_type, label, attrs), layout hints (rankdir, spacing, groups), legends (color/shape per element/relationship), and filters used.
