 ...
 * List page for Application Interface entities
 */
...
export const ApplicationInterfaceListPage: React.FC = () => {
  const navigate = useNavigate();
  const [filters, setFilters] = useState<Record<string, any>>({});
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [entityToDelete, setEntityToDelete] = useState<ApplicationInterface | null>(null);
  ...
  const handleRowAction = (action: 'view' | 'edit' | 'delete', item: ApplicationInterface) => {
    switch (action) {
      case 'view':
        navigate(`/entities/application-interfaces/${item.id}`);
        break;
      case 'edit':
        navigate(`/entities/application-interfaces/${item.id}/edit`);
        break;
      case 'delete':
  ...
  const handleDeleteConfirm = async (approvalId: string, reason: string) => {
    ...
    try {
      await deleteEntity(async () => applicationInterfacesApi.delete(entityToDelete.id, approvalId, reason));
  ...
  const handleBulkDelete = async (ids: string[]) => {
    await bulkDelete(async () => applicationInterfacesApi.bulkDelete(ids), ids);
    clearSelection();
  };

  return (
    <div className="application-interface-list-page">
      <EntityListTemplate
        title="Application Interfaces"
        description="Manage all interfaces connecting applications and systems"
        items={items}
        columns={COLUMNS}
        ...
        onCreateNew={() => navigate('/entities/application-interfaces/new')}
      ...
      <DeleteConfirmModal
        ...
        entityLabel={entityToDelete ? `application interface "${entityToDelete.name}"` : 'application interface'}
